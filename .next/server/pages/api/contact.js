...
const { MAILGUN_API_KEY, MAILGUN_DOMAIN, MAILGUN_FROM, EMAIL_TO, MAILGUN_API_BASE } = process.env;
...
async function handler(req, res) {
    ...
    if (!MAILGUN_API_KEY || !MAILGUN_DOMAIN || !EMAIL_TO || !MAILGUN_FROM) {
        ...
        return res.status(500).json({
            error: "Mailgun not configured on server"
        });
    }
  ...
  `;
    ...
    const auth = "Basic " + external_buffer_namespaceObject.Buffer.from(`api:${MAILGUN_API_KEY}`).toString("base64");
    try {
        ...
        if (!sendUser.ok) {
            ...
            return res.status(200).json({
                ok: true,
                warning: "Owner notified, failed to send confirmation to user",
                details: text
            });
        }
        return res.status(200).json({
...
var __webpack_exports__ = (__webpack_exec__(7422));
